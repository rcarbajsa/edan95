...
class GMM:
    #Initial state
    def __init__(self,data,target, X,classes,iterations):
        self.data =data
        self.target=target
    ...
    def M_step(self, r):
        r_k = {k:np.sum(r[:,k]) for k in range(self.classes)}
        #self.theta = dict()
        epsilon = 0.01
        for k in r_k:
            pi = r_k[k]/len(self.X)
